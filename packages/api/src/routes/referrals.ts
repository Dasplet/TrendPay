...
router.get('/admin', authenticate, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ok: true,
      referidos: filtered.map(r => { const comision_valor = Number.parseFloat(r.comisionValor.toString()); return { id: r.id, referidor_nombre: r.referidor.nombre, referidor_cedula: r.referidor.cedula, referido_nombre: r.referido.nombre, referido_cedula: r.referido.cedula, comision_valor, status: r.status, created_at: r.createdAt }; }),
...
router.get('/me', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ok: true,
      referidos: referrals.map(r => { const comision_valor = Number.parseFloat(r.comisionValor.toString()); return { id: r.id, nombre: r.referido.nombre, cedula: r.referido.cedula, status: r.status, comision_valor, created_at: r.createdAt }; }),
