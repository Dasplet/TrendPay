...
const MOCK_ALERTS = [
  { id:1, tipo:'Alto', titulo:'Alto volumen inusual', desc:"Retiro de $2.800.000 · CC 1023456789 · supera el 90% del límite diario", tiempo:'Hace 8 min', icon:'↑', color:'#C0392B', bg:'rgba(192,57,43,.15)' },
  { id:2, tipo:'Medio', titulo:'Intentos fallidos de PIN', desc:'3 intentos incorrectos · CC 1023456789 · dispositivo Samsung S24', tiempo:'Hace 15 min', icon:'🔒', color:'#d4a017', bg:'rgba(212,160,23,.15)' },
  { id:3, tipo:'Info', titulo:'Nuevo dispositivo detectado', desc:'Admin TrendPay · Chrome en MacBook · Medellín, Colombia', tiempo:'Hace 2 h', icon:'📱', color:'#852EC7', bg:'rgba(133,46,199,.15)' },
];
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      <Panel style={{ border:'1px solid rgba(192,57,43,.25)' }}>
        <PanelHeader title="Alertas de riesgo activas" icon="△" />
        <div style={{ padding:'12px 16px', display:'flex', flexDirection:'column', gap:10 }}>
          ...
          {alerts.map(a => (
            <div key={a.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 16px', background:'rgba(30,12,65,.5)', borderRadius:12, border:'1px solid rgba(133,46,199,.12)' }}>
              <div style={{ width:44, height:44, borderRadius:12, background:a.bg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, flexShrink:0 }}>{a.icon}</div>
              <div style={{ flex:1 }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                <Btn variant="ghost" style={{ padding:'6px 10px' }}>👁</Btn>
                <Btn variant="ghost" style={{ padding:'6px 10px' }} onClick={() => dismiss(a.id)}>✓</Btn>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Antifraud params */}
      <Panel>
        <PanelHeader title="Parámetros antifraude" icon="⊙" />
          ...
          {[
            ...
            <div key={p.key} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:'1px solid rgba(255,255,255,.04)' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                <Btn variant="ghost" style={{ padding:'5px 8px' }}>✏</Btn>
              </div>
            </div>
          ))}
        </div>
