...
function ParamRow({ label, sub, value, onChange }: any) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 24px', borderBottom:'1px solid rgba(255,255,255,.05)' }}>
      <div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        {editing ? (
          <input value={val} onChange={e=>setVal(e.target.value)} onBlur={()=>{setEditing(false);onChange(val);}}
            style={{ width:80, background:'rgba(30,12,65,.8)', border:'1.5px solid #852EC7', borderRadius:8, padding:'6px 10px', fontSize:14, fontWeight:700, color:'#fff', textAlign:'center', outline:'none' }} autoFocus />
        ) : (
          <div style={{ background:'rgba(30,12,65,.7)', border:'1px solid rgba(133,46,199,.2)', borderRadius:8, padding:'6px 14px', fontSize:14, fontWeight:700, color:'#fff', minWidth:80, textAlign:'center', cursor:'pointer' }} onClick={()=>setEditing(true)}>
            {val}
          </div>
        )}
    ...
    <div style={{ display:'grid', gridTemplateColumns:'1.2fr 1fr', gap:16 }}>
      ...
      <Panel>
        ...
        <div style={{ padding:'8px 0', maxHeight:500, overflowY:'auto' }}>
          ...
          {users.length===0 && <div style={{ textAlign:'center', padding:32, color:'rgba(255,255,255,.3)', fontSize:13 }}>Sin usuarios</div>}
        </div>
      </Panel>
    </div>
  );
