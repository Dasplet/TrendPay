...
function MetricCard({ icon, value, label, color, badge }: any) {
  ...
  return (
    <div style={{ background:gradients[color]||gradients.purple, borderRadius:16, padding:'28px 24px', position:'relative', overflow:'hidden', border:'1px solid rgba(255,255,255,.05)' }}>
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div style={{ background:'rgba(20,10,50,.97)', border:'1px solid rgba(133,46,199,.35)', borderRadius:10, padding:'10px 14px', fontSize:12 }}>
      <div style={{ color:'#AE93AA', marginBottom:6 }}>{label}</div>
...
export default function AdminDashboard() {
  ...
  const barColors=['#852EC7','#6CC998','#AE93AA','#C0392B','#d4a017','#4a90d9','#e87575','#7cc47c'];
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
        ...
        <div style={{ padding:'16px 18px 10px', height:280 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={pts} margin={{ top:10, right:20, left:10, bottom:5 }}>
              ...
              <XAxis dataKey="name" tick={{ fill:'rgba(174,147,170,.55)', fontSize:10 }} axisLine={false} tickLine={false} interval={step} />
              <YAxis tickFormatter={fmtAxis} tick={{ fill:'rgba(174,147,170,.55)', fontSize:10 }} axisLine={false} tickLine={false} />
              <Tooltip content={<ChartTooltip />} />
              <Legend wrapperStyle={{ fontSize:11, color:'rgba(174,147,170,.8)', paddingTop:8 }} />
        ...
        <div style={{ padding:'16px 18px 10px', height:220 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={pts} margin={{ top:5, right:20, left:10, bottom:5 }}>
              ...
              <Bar dataKey="envio"    name="Envíos"    stackId="a" fill="rgba(174,147,170,.75)" />
              ...
              return (
                <div key={u.id} style={{ marginBottom:12 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:4 }}>
                    <span style={{ color:'rgba(255,255,255,.7)' }}>{u.nombre?.split(' ')[0]||'—'}</span>
          ...
          <div style={{ padding:'8px 0' }}>
            {[['API Backend'],['Base de datos'],['ACH Colombia'],['Rapyd'],['PSE Débito']].map(([n])=>(
              <div key={n} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'13px 20px', borderBottom:'1px solid rgba(255,255,255,.04)' }}>
                <span style={{ fontSize:13, color:'rgba(255,255,255,.75)' }}>{n}</span>
      ...
      <Panel>
        ...
        <Table headers={['Referencia','Descripción','Usuario','Monto','Estado','Fecha']}>
          {txs.length===0 ? (
            <TR><TD style={{ textAlign:'center', padding:32, color:'rgba(255,255,255,.3)' }} colSpan={6 as any}>Sin transacciones</TD></TR>
          ):txs.slice(0,8).map((t:any)=>(
            <TR key={t.id}>
              <TD style={{ fontFamily:'monospace', fontSize:11, color:'rgba(255,255,255,.4)' }}>{(t.codigo||'—').slice(0,14)}</TD>
              <TD style={{ fontSize:12, maxWidth:280, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.descripcion}</TD>
              <TD style={{ fontSize:12, color:'rgba(255,255,255,.65)' }}>{(t.usuario_nombre||'—').split(' ')[0]}</TD>
